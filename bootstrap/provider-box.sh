...
require_keycloak_vars() {
  local var
  for var in WORKDIR KEYCLOAK_DIR KEYCLOAK_FQDN KEYCLOAK_IMAGE KEYCLOAK_ADMIN_USER KEYCLOAK_ADMIN_PASSWORD KEYCLOAK_BOOTSTRAP_REALM_NAME KEYCLOAK_BOOTSTRAP_GROUP_NAME KEYCLOAK_BOOTSTRAP_CLIENT_ID KEYCLOAK_BOOTSTRAP_CLIENT_SECRET KEYCLOAK_BOOTSTRAP_CLIENT_REDIRECT_URIS; do
    [[ -n "${!var:-}" ]] || fail "Missing required variable: $var"
  done
